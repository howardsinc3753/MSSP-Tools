...
- **A FortiGate** with REST API access -- see [API Token Setup](#fortigate-api-token-setup) below
- **An Anthropic API key** -- [get one here](https://console.anthropic.com/) (free tier available)

### FortiGate API Token Setup

You need a REST API token from your FortiGate before starting. This takes about 2 minutes:
...
> **Tip:** If you don't see "REST API Admin" as an option, enable it under **System > Feature Visibility > REST API**.

### Install

```bash
git clone https://github.com/howardsinc3753/MSSP-Tools.git
cd MSSP-Tools/FortiBot-NOC-CLI
pip install .
```

If you already have the repo cloned, just `cd FortiBot-NOC-CLI && pip install .`

If you hit dependency issues, install them explicitly first:
...
# Windows
fortibot backup --path C:\backups\
...
export FORTIBOT_MODEL="claude-haiku-4-5-20251001"

# Windows (CMD)
set ANTHROPIC_API_KEY=sk-ant-api03-...
set FORTIBOT_MODEL=claude-haiku-4-5-20251001

# Windows (PowerShell)
$env:ANTHROPIC_API_KEY = "sk-ant-api03-..."
$env:FORTIBOT_MODEL = "claude-haiku-4-5-20251001"
...
- All diagnostic data is sent to Claude API for analysis -- review Anthropic's [data handling policy](https://www.anthropic.com/policies) for your compliance requirements
...
- **Scheduled Diagnostics** -- Cron-style health checks with alerting on threshold violations
- **Device Groups** -- Batch commands across 10+ FortiGates (e.g., `--group all-branches`)
- **Report Export** -- PDF/CSV/HTML diagnostic reports for management and compliance
- **Audit Logging** -- Structured logs (JSON/syslog) of who ran what, when, from where
- **Secrets Manager Integration** -- HashiCorp Vault, AWS Secrets Manager, or OS keyring
